...
class CryptoBloc extends Bloc<CryptoEvent, CryptoState> {
  ...
  Stream<CryptoState> mapEventToState(CryptoEvent event) async* {
    ...
    } else if (event is CryptoErrorRetryEvent) {
      yield* _mapCrytpoErrorRetryToState();
    }
  }
  ...
  Stream<CryptoState> _getCoins({List<Coin> coins, int page = 0}) async* {
    ...
    try {
      List<Coin> newCoinsList =
          coins + await _cryptoRepository.getTopCoins(page);
  ...
  Stream<CryptoState> _mapCrytpoErrorRetryToState() async* {
    yield CryptoLoading();
    yield* _getCoins(coins: []);
  }
}
